...
class UrlsController {
    ...
    try {
      ...
      if (!validationErorrs.isEmpty()) {
        return res
          .status(400)
          .send(
            `Json body validation error. ${JSON.stringify(validationErorrs)}`
          );
      }
      ...
      res
        .status(200)
        .json({
          shortedUrl: `${dbRespocne.hostUrl}/${dbRespocne.shortUrlPath}`,
        });
    } catch (err) {
      next(err);
    }
  ...
  async getUrl(req: Request, res: Response, next: NextFunction) {
    try {
      const shortedUrl = await ShortUrl.findOne({
        shortUrlPath: req.params.shortedUrl,
      });
