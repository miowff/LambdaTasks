      ...
      for (let j = 0; j < rowLength && i < currencies.length; j++) {
        row.push(
          this.createInlineButton(
            ...
            JSON.stringify(new CallbackDataQuery('/Info', currencies[i].Id)),
          ),
        );
        i++;
      }
      ...
      [
        {
          ...
          callback_data: JSON.stringify(
            new CallbackDataQuery(this.returnRoute),
          ),
        },
        {
          text: 'Детали',
          callback_data: JSON.stringify(
            new CallbackDataQuery('/Details', currency.Id),
          ),
        },
      ],
      [
        isInFavourites
          ? {
              ...
              callback_data: JSON.stringify(
                new CallbackDataQuery('/RemoveFavourite', currency.Id),
              ),
            }
          : {
              text: 'В избранное',
              callback_data: JSON.stringify(
                new CallbackDataQuery('/AddToFavourite', currency.Id),
              ),
            },
      ],
    ];
