...
describe("Min deadline tests", () => {
  ...
  testCases.forEach((testCase) => {
    it(`Request date ${testCase.requestDate.toDateString()}.
                Language: ${testCase.requestObject.language} Mimetype: ${
      testCase.requestDate.mimetype
    }
                Should return ${testCase.expectedDeadlineDate}`, () => {
      //Arrange & Act
      var result = deadlineCounter(
        testCase.requestObject,
        testCase.requestDate
      );
      //Assert
...
describe("Is hours correct for other mimetype RU and UA", () => {
  ...
  testCases.forEach((testCase) => {
    it(`Request date ${testCase.requestDate.toDateString()}.
      ...
      //Arange
      var expectedValue =
        (testCase.requestObject.count / RU_UA_SPEED + 0.5) * 1.2;
      //Act
      var result = deadlineCounter(
        testCase.requestObject,
        testCase.requestDate
