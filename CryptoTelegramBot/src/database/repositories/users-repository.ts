...
class UsersRepository extends BaseRepository<UserModel> {
  async getFavouriteCurrenciesAsync(userId: number): Promise<CurrencyModel[]> {
    return new Promise((resolve) => {
      db.all(
        `SELECT * FROM currencies JOIN userCurrency on currencies.id = CurrencyId where userId = ${userId}`,
        [],
        (err, rows) => {
          if (err) {
            throw err;
  ...
  async removeCurrencyFromFavourites(currencyId: string, userId: number) {
    await db.run(
      `DELETE FROM userCurrency WHERE CurrencyId = '${currencyId}' AND UserId = ${userId}`,
    );
  }
}
