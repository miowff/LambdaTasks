...
class BotCallbackQueriesHandler {
  async handleCallbackQueryAsync(callbackQuery: CallbackQuery) {
      ...
      switch (query.action) {
        case '/BackToMenu': {
          ...
          const options: SendMessageOptions = {
            reply_markup: {
              inline_keyboard: inlineKeyboardService.createInlineKeyboard(
                aveliableCurrencies,
                4,
              ),
            },
        ...
        case '/BackToFavourites': {
          const favouriteCurencies =
            await usersRepository.getFavouriteCurrenciesAsync(chatId);
          const options: SendMessageOptions = {
            reply_markup: {
              inline_keyboard: inlineKeyboardService.createInlineKeyboard(
                favouriteCurencies,
    ...
    switch (query.action) {
      case '/Info': {
        const priceData = new PriceDataModel(
          currency.Id,
          currency.CurrencyCode,
          fifteenMinPrice,
        );

        const isInFavourites = await currenciesRepository.isInFavouritesAsync(
          chatId,
          currency.Id,
        );
        ...
        const options: SendMessageOptions = {
          reply_markup: {
            inline_keyboard: inlineKeyboardService.currencyInlineKeyboard(
              currency,
              isInFavourites,
            ),
          },
              ...
              [
                {
                  ...
                  callback_data: JSON.stringify(
                    new CallbackDataQuery('/Info', currency.Id),
                  ),
                },
              ],
            ],
