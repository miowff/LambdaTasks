...
{
    private cognitoClient = new AWS.CognitoIdentityServiceProvider({region:process.env.REGION});
    ...
    {
        const responce = await this.cognitoClient.initiateAuth(signInRequest).promise();
        return responce;
    }
}
