...
export const handler = async (
  ...
  try {
    ...
    const existigMarket = await marketsRepository.getByIdAsync(
      bodyData.MarketId
    );
    if (!existigMarket) {
      return {
        statusCode: 400,
        body: JSON.stringify(
          `There is no market with id: ${bodyData.MarketId}`
        ),
      };
    }
    const marketRequests = await marketRequestsRepository.getByIdAsync(
      bodyData.MarketId
    );
    if (existigMarket.MaxRrequestsCount <= marketRequests.RequestsCount) {
      return {
        statusCode: 400,
        body: JSON.stringify(`Requests limit is over`),
      };
    }
